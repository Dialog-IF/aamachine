...
// translit in a2_frontend.s scans the table with ldx #NTRANS-1 / dex / bpl,
...
void put_table(FILE *f, const char *label, struct entry **tr, int ntr, int which) {
	int i;
	...
	for(i = 0; i < ntr; i++) {
		...
		case 3: put_char_byte(f, tr[i]->translit[1]); break;
		}
		if((i & 7) == 7) fprintf(f, "\n");
	}
	if(ntr & 7) fprintf(f, "\n");
...
// Emit the trhi/trlo/trc0/trc1 arrays read by translit in a2_frontend.s.
// NTRANS there is trlo-trhi, so the four arrays must stay the same length
// and in this order.
...
void write_translit(const char *filename) {
	...
	put_table(f, "trhi", tr, ntr, 0);
	put_table(f, "trlo", tr, ntr, 1);
	put_table(f, "trc0", tr, ntr, 2);
	put_table(f, "trc1", tr, ntr, 3);
