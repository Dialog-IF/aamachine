...
* `SHORT`: Number of word-to-object maps
** `SHORT`: Byte offset in chunk of word-to-object map
...
* `SHORT`: Number of style classes
** `SHORT`: Byte offset in chunk of style definition

Each style definition consists of zero or more null-terminated ASCII
strings. Each is a CSS key-value pair without the final semicolon, e.g.
...
except that $10 (normally undefined for output) is a line feed.

In 32-bit story files, these strings use UTF-8. Line feeds should always
be $10, not $13.
...
* `SHORT`: Number of resources
** `SHORT`: Byte offset in chunk of descriptor
...
following bytes. This opcode is always illegal in 16-bit story files,
and thus 16-bit interpreters need not try to decode it.
...
object (or null) given in arg1, then extracts the (arg2 % BITS_PER_WORD)th
...
This is the value such that ((P/Q) * Q) + (P % Q) = P. Since division rounds
toward negative infinity, that means P % Q has the sign of Q, not the sign
of P - this is a modulo operation, not a remainder operation. In other words,
-5 % 2 = 1, not -1 (but 5 % -2 = -1).
